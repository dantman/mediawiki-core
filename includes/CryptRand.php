...
final class MWCryptRand {
	...
	/**
	 * Number of rounds we should make when testing
	 * how long drift calculation takes.
	 */
	const NUM_TEST_ROUNDS = 50;

	/**
	 * Estimated number of bits of entropy we get from each
	 * clock drift calculation we run
	 */
	const BITS_PER_CALC = 2;

	/**
	 * Number of milliseconds we want to spend generating each separate byte
	 * of the final generated bytes.
	 * This is used in combination with the hash length to determine in-part
	 * how long a single clock drift calculations should take.
	 */
	const MSEC_PER_BYTE = 25;

	/**
	 * Return the parameters to use for clock drift calculations (iterations, rounds)
	 *
	 * @return Array array( $iterations, $rounds )
	 */
	private static function getDriftParameters() {
		static $ret = null;
		if ( !is_null( $ret ) ) {
			return $ret;
		}
		// Measure a guess of how long the clock drift operation will take
		$seconds = array();
		list ( , $seconds[] ) = self::runDriftCalculation( self::NUM_TEST_ROUNDS );
		list ( , $seconds[] ) = self::runDriftCalculation( self::NUM_TEST_ROUNDS );
		list ( , $seconds[] ) = self::runDriftCalculation( self::NUM_TEST_ROUNDS );
		$seconds = array_sum( $seconds ) / count( $seconds );
		// Calculate the number of clock drift iterations we should make
		// Base this on how many bytes we'll be outputting in the final output
		// and how many bits of entropy each clock drift calculation makes
		// - Given an estimate of how many bits of entropy each iteration gives
		//   multiply that by 8 to get the number of iterations needed for a single
		//   byte of entropy.
		// - Multiply this by the number of bytes the final output will add (the hash length)
		$iterations = self::hashLength() * ( self::BITS_PER_CALC * 8 );
		// Calculate the number of rounds we should make in the clock drift calculation
		// - Calculate the average number of milliseconds a single round takes
		// - Calculate how many milliseconds we want to spend on one bit of entropy
		// - Use that to calculate how many milliseconds each iteration should take
		// - Calculate rounds by dividing iteration time by average time for one round
		$msAdvPerRound = ( $seconds / self::NUM_TEST_ROUNDS ) * 1000;
		$msDesirePerBit = ceil( self::MSEC_PER_BYTE / 8 );
		$msDesirePerIter = $msDesirePerBit / self::BITS_PER_CALC;
		$rounds = ceil( $msDesirePerIter / $msAdvPerRound );
		wfDebug( __METHOD__ . ": Clock drift calculation parameters (iterations=$iterations, rounds=$rounds)" .
			" input (hash-length=" . self::hashLength() . ", average-round-ms=$msAdvPerRound)" .
			" time estimate " . ( $iterations * $rounds * $msAdvPerRound ) . "ms per " . self::hashLength() . "byte chunk\n" );
		return $ret = array( $iterations, $rounds );
	}

	/**
	 * Run a clock drift calculation.
	 *
	 * @param $rounds The number of hash rounds to run for the clock drift calculation
	 * @return Array array( $endSeconds, $elapsedSeconds )
	 */
	private static function runDriftCalculation( $rounds ) {
		// Store the start time
		$start = microtime( true );
		// Run the hash algorithm as many times specified by $rounds
		$var = self::hash( mt_rand() );
		for ( $i = 0; $i < $rounds; $i++ ) {
			$var = self::hash( $var );
		}
		// Get the end time
		$end = microtime( true );
		// Return the end time and the elapsed number of seconds
		return array( $end, $end - $start );
	}

	/**
	 * Randomly hash data while mixing in clock drift data for randomness
	 *
	 * Runs a number of clock drift calculation rounds mixing the entropy
	 * from the clock drift into the inputted data in the form of a
	 * hmac key.
	 *
	 * @param $data The data to randomly hash.
	 * @return String The hashed bytes
	 */
	private static function driftHash( $data ) {
		list ( $iterations, $rounds ) = self::getDriftParameters();
		for ( $i = 0; $i < $iterations; $i++ ) {
			list( $end, $seconds ) = self::runDriftCalculation( $rounds );
			...
			// clock drift as the key and the larger state as the data.
			// Additionally if hex2bin is available we try to reduce the string of
			// decimals to the raw binary bytes that hmac takes as key input
			$key = str_replace( '.', '', $end . ( $seconds * 1000 ) );
			if ( function_exists( 'hex2bin' ) ) {
				$key = hex2bin( wfBaseConvert( $key, 10, 16 ) );
			}
			$data = self::hmac( $data, $key );
		}
		return $data;
	}
	...
	private static function randomState() {
		...
		if ( is_null( $state ) ) {
			...
			$state = self::hash( self::initialRandomState() );
		}
		// Generate a new random state based on the initial random state or previous
		// random state by combining it with clock drift
		$state = self::driftHash( $state );
		return $state;
	}
	 ...
	 */
	private static function hashLength() {
		static $hashLength = null;
		if ( is_null( $hashLength ) ) {
			$hashLength = strlen( self::hash( '' ) );
		}
		return $hashLength;
	}
