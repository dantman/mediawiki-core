...
abstract class Skin extends ContextSource {
	...
	static function &newFromKey( $key ) {
		...
		if ( $skinName[0] == '{' ) {
			$className = preg_replace( '/^\{([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}.*$/', '$1', $skinName );
		} else {
		...
		if ( !MWInit::classExists( $className ) ) {
			...
			if ( !defined( 'MW_COMPILED' ) && $skinName[0] != '{' ) {
			...
			if ( !MWInit::classExists( $className ) || $skinName[0] == '{' ) {
	...
	protected function init() {
		...
		$oldStylename = $this->stylename;
		$this->initPage( $this->getOutput() );
		if ( $this->skinname != $oldSkinname || $this->stylename != $oldStylename ) {
			wfDeprecated( "Please define the skinname and skinkey directly as class variables instead of changing the value within initPage." );
		}
	 ...
	 * ((This method is old, please override Skin::execute() in new subclasses instead of this one))
	 * @param $out OutputPage
	 */
	public function outputPage( OutputPage $out = null ) {
		...
		// Execute it and output the html
		$this->execute();
	...
	/**
	 * Execute the skin.
	 * Subclasses should output the page's html from this method.
	 * @abstract
	 */
	protected function execute() {
		// Previous versions of MediaWiki had outputPage as the primary abstract function and subclasses overrode that
		// for that reason we can't make execute() abstract. Instead we throw a generic php exception.
		throw new LogicException( get_class( $this ) . " did not implement an *::execute(); method to override Skin::execute();" );
	}
